...
public static class MetaverseSceneBuilder
{
    ...
    class FieldArea
    {
        ...
        public Color Ground, Wall, Rock, Wood, Tuft, Prop, Portal;
    ...
    class DungeonArea
    {
        ...
        public int Theme;
        public int Required;
        public int ChestGold, ChestExp, ChestOre;
        public Color Floor, Stone, Torch, Portal;
    ...
    static readonly FieldArea[] FieldAreas =
    {
        new FieldArea
        {
            Name = "초원 사냥터", Center = new Vector3(120f, 0f, 0f),
            Ground = new Color(0.28f, 0.38f, 0.26f), Wall = new Color(0.30f, 0.32f, 0.38f),
            Rock = new Color(0.44f, 0.46f, 0.42f), Wood = new Color(0.34f, 0.27f, 0.21f),
            Tuft = new Color(0.42f, 0.58f, 0.28f), Prop = new Color(0.44f, 0.46f, 0.42f),
        ...
        new FieldArea
        {
            ...
            Tuft = new Color(0.90f, 0.95f, 1f), Prop = new Color(0.66f, 0.86f, 0.96f),
        ...
        new FieldArea
        {
            ...
            Tuft = new Color(0.85f, 0.35f, 0.15f), Prop = new Color(1f, 0.45f, 0.12f),
    ...
    static readonly DungeonArea[] DungeonAreas =
    {
        new DungeonArea
        {
            Name = "폐허 던전", Center = new Vector3(-120f, 0f, 0f),
            GuardBonus = 3, Required = 5,
            ChestGold = 90, ChestExp = 40, ChestOre = 2,
            Floor = new Color(0.24f, 0.22f, 0.26f), Stone = new Color(0.34f, 0.32f, 0.36f),
            Torch = new Color(0.98f, 0.65f, 0.25f), Portal = new Color(0.65f, 0.30f, 0.90f),
        },
        new DungeonArea
        {
            ...
            Torch = new Color(0.55f, 0.85f, 1f), Portal = new Color(0.55f, 0.85f, 1f),
        },
        new DungeonArea
        {
            ...
            Torch = new Color(1f, 0.42f, 0.12f), Portal = new Color(1f, 0.42f, 0.12f),
        },
    };
    ...
    static void BuildWorld()
    {
        ...
        var fieldPad = BuildWarpPad(world.transform, "WarpPadVillage", VillagePad, "사냥터", FieldAreas[0].Arrival, "PortalField", FieldAreas[0].Portal);
        fieldPad.Choices = System.Array.ConvertAll(FieldAreas, area => new WarpPad.Choice
        {
            Name = area.Name,
        ...
        var dungeonPad = BuildWarpPad(world.transform, "WarpPadDungeon", VillageDungeonPad, "던전", DungeonAreas[0].Arrival, "PortalDungeon", DungeonAreas[0].Portal);
        dungeonPad.Choices = System.Array.ConvertAll(DungeonAreas, area => new WarpPad.Choice
        {
            Name = area.Name,
            Destination = area.Arrival,
            RequiredLevel = area.Required,
    ...
    static void BuildHouse(Transform parent, string name, Vector3 groundPosition, Vector3 size)
    {
        ...
        CreatePrimitive(PrimitiveType.Cube, "RoofLeft", house.transform, new Vector3(-size.x * 0.22f, roofY, 0f), new Vector3(size.x * 0.62f, 0.3f, size.z + 1.2f), roofMaterial, Quaternion.Euler(0f, 0f, 22f));
        CreatePrimitive(PrimitiveType.Cube, "RoofRight", house.transform, new Vector3(size.x * 0.22f, roofY, 0f), new Vector3(size.x * 0.62f, 0.3f, size.z + 1.2f), roofMaterial, Quaternion.Euler(0f, 0f, -22f));
    ...
    static void BuildArena()
    {
        ...
        for (int i = 0; i < 12; i++)
        {
            ...
            CreatePrimitive(PrimitiveType.Cube, $"RingWall{i}", arena.transform,
                new Vector3(Mathf.Cos(angle) * ArenaRadius, 0.85f, Mathf.Sin(angle) * ArenaRadius),
                new Vector3(ArenaRadius * 0.58f, 1f, 0.6f), trimMaterial, Quaternion.Euler(0f, -angle * Mathf.Rad2Deg + 90f, 0f));
        }
        ...
        BuildWarpPad(arena.transform, "WarpPadArenaExit", ArenaPad - ArenaCentre, "마을", VillageArenaArrival, "PortalVillage", new Color(0.30f, 0.80f, 0.85f));
    ...
    static void BuildAnvil(Transform parent, Vector3 localPosition, Material woodMaterial)
    {
        ...
        BodyPart(PrimitiveType.Cube, "Horn", anvil.transform, new Vector3(0.78f, 1.19f, 0f), new Vector3(0.55f, 0.13f, 0.24f), ironMaterial, Quaternion.Euler(0f, 0f, -6f));

        // Hammer resting across the plate.
        BodyPart(PrimitiveType.Cube, "HammerHandle", anvil.transform, new Vector3(-0.28f, 1.32f, 0.05f), new Vector3(0.07f, 0.5f, 0.07f), handleMaterial, Quaternion.Euler(0f, 20f, 78f));
    ...
    static void BuildCampfire(Transform parent, Vector3 localPosition, Material woodMaterial, Material emberMaterial)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            CreatePrimitive(PrimitiveType.Sphere, $"Stone{i}", fire.transform,
                new Vector3(Mathf.Cos(angle) * 1.05f, 0.16f, Mathf.Sin(angle) * 1.05f),
                new Vector3(0.42f, 0.32f, 0.42f), stoneMaterial, Quaternion.Euler(0f, i * 31f, 0f));
        }

        // Crossed logs in the middle.
        for (int i = 0; i < 3; i++)
        {
            BodyPart(PrimitiveType.Cylinder, $"Log{i}", fire.transform, new Vector3(0f, 0.3f, 0f), new Vector3(0.16f, 0.6f, 0.16f), woodMaterial, Quaternion.Euler(62f, i * 60f, 0f));
        }

        // Flames: three shrinking blocks, turned so they never look like a stacked box.
        BodyPart(PrimitiveType.Cube, "FlameLow", fire.transform, new Vector3(0f, 0.55f, 0f), new Vector3(0.7f, 0.7f, 0.7f), emberMaterial, Quaternion.Euler(0f, 25f, 0f));
        BodyPart(PrimitiveType.Cube, "FlameMid", fire.transform, new Vector3(0.06f, 0.95f, -0.04f), new Vector3(0.48f, 0.55f, 0.48f), emberMaterial, Quaternion.Euler(0f, -15f, 8f));
        BodyPart(PrimitiveType.Cube, "FlameTip", fire.transform, new Vector3(-0.05f, 1.28f, 0.05f), new Vector3(0.28f, 0.38f, 0.28f), flameMaterial, Quaternion.Euler(0f, 40f, -10f));

        // Tripod with a pot hanging over the fire.
        for (int i = 0; i < 3; i++)
        {
            float angle = i * Mathf.PI * 2f / 3f;
            BodyPart(PrimitiveType.Cube, $"TripodLeg{i}", fire.transform,
                new Vector3(Mathf.Cos(angle) * 0.55f, 0.9f, Mathf.Sin(angle) * 0.55f),
                new Vector3(0.09f, 1.9f, 0.09f), woodMaterial, Quaternion.Euler(Mathf.Sin(angle) * 18f, 0f, -Mathf.Cos(angle) * 18f));
        }
        ...
        foreach (float side in new[] { -1f, 1f })
        {
            CreatePrimitive(PrimitiveType.Cylinder, side < 0f ? "SeatLeft" : "SeatRight", fire.transform,
                new Vector3(side * 2f, 0.25f, side * 0.4f), new Vector3(0.5f, 1.1f, 0.5f), woodMaterial, Quaternion.Euler(90f, side * 20f, 0f));
        }
    ...
    static void BuildGatherNode(Transform parent, string name, Vector3 localPosition, GatherKind kind)
    {
        ...
        switch (kind)
        {
            case GatherKind.Ore:
                ...
                BodyPart(PrimitiveType.Cube, "Crystal", node.transform, new Vector3(-0.35f, 0.85f, 0.1f), new Vector3(0.28f, 1f, 0.28f), oreCrystalMaterial, Quaternion.Euler(0f, 20f, -18f));
                BodyPart(PrimitiveType.Cube, "Crystal", node.transform, new Vector3(0.3f, 1f, -0.15f), new Vector3(0.24f, 1.3f, 0.24f), oreCrystalMaterial, Quaternion.Euler(12f, -30f, 14f));
                BodyPart(PrimitiveType.Cube, "Crystal", node.transform, new Vector3(0.05f, 0.75f, 0.45f), new Vector3(0.2f, 0.8f, 0.2f), oreCrystalMaterial, Quaternion.Euler(-16f, 5f, 22f));
                break;
    ...
    static WarpPad BuildWarpPad(Transform parent, string name, Vector3 position, string label, Vector3 destination, string portalName, Color portalColor)
    {
        ...
        BodyPart(PrimitiveType.Cube, "Rune", pad.transform, new Vector3(0f, 5.2f, 0f), new Vector3(0.55f, 0.55f, 0.55f), portalMaterial, Quaternion.Euler(45f, 45f, 0f));
    ...
    static void BuildDungeon(GameObject monsterPrefab, DungeonArea area)
    {
        ...
        BuildWarpPad(dungeon.transform, "WarpPadDungeonExit" + area.Key, new Vector3(0f, 0.05f, -26f), "마을", VillageDungeonArrival, "PortalVillage", new Color(0.30f, 0.80f, 0.85f));
    }
    ...
    static void BuildHuntingField(GameObject monsterPrefab, FieldArea area)
    {
        ...
        for (int i = 0; i < boulders.Length; i++)
        {
            ...
            CreatePrimitive(PrimitiveType.Sphere, $"Boulder{i}", field.transform,
                new Vector3(boulders[i].position.x, size * 0.3f, boulders[i].position.z),
                new Vector3(size, size * 0.75f, size * 0.9f),
                bouldMaterial, Quaternion.Euler(0f, i * 37f, 8f));
        ...
        for (int i = 0; i < deadTrees.Length; i++)
        {
            ...
            BodyPart(PrimitiveType.Cylinder, "BranchLeft", tree.transform, new Vector3(-0.7f, 3f, 0f), new Vector3(0.2f, 0.9f, 0.2f), deadWoodMaterial, Quaternion.Euler(0f, 0f, 55f));
            BodyPart(PrimitiveType.Cylinder, "BranchRight", tree.transform, new Vector3(0.7f, 3.4f, 0.1f), new Vector3(0.18f, 0.8f, 0.18f), deadWoodMaterial, Quaternion.Euler(10f, 0f, -50f));
        }
        ...
        for (int i = 0; i < 14; i++)
        {
            ...
            BodyPart(PrimitiveType.Cube, $"Grass{i}", field.transform,
                new Vector3(Mathf.Cos(angle) * radius, 0.25f, Mathf.Sin(angle) * radius),
                new Vector3(0.5f, 0.5f, 0.5f), tuftMaterial, Quaternion.Euler(0f, i * 23f, 12f));
        }
        ...
        BuildWarpPad(field.transform, "WarpPadField" + area.Key, new Vector3(0f, 0.05f, -25f),
            "마을", VillageArrival, "PortalVillage", new Color(0.30f, 0.80f, 0.85f));
    ...
    static void BuildIceSpikes(Transform parent, Material iceMaterial)
    {
        var spots = new[]
        {
            new Vector3(-6f, 0f, 6f), new Vector3(16f, 0f, 10f), new Vector3(-18f, 0f, -6f),
            new Vector3(2f, 0f, -10f), new Vector3(22f, 0f, -18f),
        };
        for (int i = 0; i < spots.Length; i++)
        {
            ...
            for (int shard = 0; shard < 3; shard++)
            {
                ...
                BodyPart(PrimitiveType.Cube, $"Shard{shard}", cluster.transform,
                    new Vector3((shard - 1) * 0.8f, height * 0.5f, shard * 0.5f),
                    new Vector3(0.55f, height, 0.55f), iceMaterial, Quaternion.Euler(shard * 7f, 45f, (shard - 1) * 11f));
            }
        }
    }
    ...
    static void BuildLavaPools(Transform parent, Material lavaMaterial, Material rockMaterial)
    {
        var spots = new[]
        {
            new Vector3(-7f, 0f, 7f), new Vector3(15f, 0f, 12f), new Vector3(-16f, 0f, -8f),
            new Vector3(3f, 0f, -12f), new Vector3(21f, 0f, -16f),
        };
        for (int i = 0; i < spots.Length; i++)
        {
            float size = 4f + (i % 3) * 1.6f;
            CreateDisc($"LavaPool{i}", parent, spots[i] + new Vector3(0f, 0.03f, 0f), size, lavaMaterial);

            BodyPart(PrimitiveType.Cylinder, $"Spire{i}", parent,
                spots[i] + new Vector3(size * 0.55f, 1.8f, size * 0.35f),
                new Vector3(0.7f, 1.8f, 0.7f), rockMaterial, Quaternion.Euler(0f, i * 33f, 6f));
        }
    }
    ...
    static GameObject BuildMonsterPrefab()
    {
        ...
        var bodies = new[]
        {
            ...
            BuildScorpionBody(root.transform, bodyMaterial, eyeMaterial, clawMaterial, emberMaterial),
            ...
            BuildLavaBruteBody(root.transform, bodyMaterial, eyeMaterial, emberMaterial, clawMaterial),
        };
    ...
    /// </summary>
    static GameObject BuildHoundBody(Transform parent, Material bodyMaterial, Material eyeMaterial, Material boneMaterial, Material iceMaterial)
    {
        var body = new GameObject("HoundBody");
        body.transform.SetParent(parent, false);

        BodyPart(PrimitiveType.Cube, "Torso", body.transform, new Vector3(0f, 0.72f, -0.15f), new Vector3(0.5f, 0.46f, 1f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Haunch", body.transform, new Vector3(0f, 0.76f, -0.6f), new Vector3(0.56f, 0.52f, 0.42f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Chest", body.transform, new Vector3(0f, 0.76f, 0.4f), new Vector3(0.6f, 0.56f, 0.42f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Neck", body.transform, new Vector3(0f, 0.88f, 0.62f), new Vector3(0.4f, 0.36f, 0.3f), bodyMaterial);
        BodyPart(PrimitiveType.Sphere, "Head", body.transform, new Vector3(0f, 0.99f, 0.8f), new Vector3(0.42f, 0.4f, 0.48f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Snout", body.transform, new Vector3(0f, 0.9f, 1.08f), new Vector3(0.22f, 0.19f, 0.34f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Jaw", body.transform, new Vector3(0f, 0.79f, 1.04f), new Vector3(0.2f, 0.12f, 0.3f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Brow", body.transform, new Vector3(0f, 1.12f, 0.92f), new Vector3(0.34f, 0.1f, 0.2f), bodyMaterial);

        // Ruff around the neck, three overlapping tufts.
        for (int i = 0; i < 3; i++)
        {
            BodyPart(PrimitiveType.Cube, $"Ruff{i}", body.transform, new Vector3((i - 1) * 0.25f, 0.92f, 0.5f), new Vector3(0.3f, 0.42f, 0.3f), bodyMaterial, Quaternion.Euler(0f, 0f, (i - 1) * 24f));
        }
        ...
        for (int i = 0; i < 4; i++)
        {
            BodyPart(PrimitiveType.Cube, $"DetailSpine{i}", body.transform,
                new Vector3(0f, 0.98f, 0.2f - i * 0.34f), new Vector3(0.12f, 0.42f - i * 0.06f, 0.12f), iceMaterial, Quaternion.Euler(-18f - i * 5f, 45f, 0f));
        }
        ...
        foreach (float side in new[] { -1f, 1f })
        {
            ...
            BodyPart(PrimitiveType.Cube, "Ear" + suffix, body.transform, new Vector3(side * 0.16f, 1.22f, 0.72f), new Vector3(0.1f, 0.26f, 0.1f), bodyMaterial, Quaternion.Euler(-14f, 0f, side * 14f));

            BodyPart(PrimitiveType.Sphere, "Eye" + suffix, body.transform, new Vector3(side * 0.14f, 1.03f, 1f), new Vector3(0.1f, 0.1f, 0.06f), iceMaterial);
            BodyPart(PrimitiveType.Cube, "Tusk" + suffix, body.transform, new Vector3(side * 0.07f, 0.8f, 1.2f), new Vector3(0.05f, 0.13f, 0.05f), boneMaterial);

            BodyPart(PrimitiveType.Cube, "LegFront" + suffix, body.transform, new Vector3(side * 0.2f, 0.34f, 0.44f), new Vector3(0.16f, 0.6f, 0.18f), bodyMaterial);
            BodyPart(PrimitiveType.Cube, "LegBack" + suffix, body.transform, new Vector3(side * 0.22f, 0.34f, -0.52f), new Vector3(0.18f, 0.6f, 0.2f), bodyMaterial);
            BodyPart(PrimitiveType.Cube, "PawFront" + suffix, body.transform, new Vector3(side * 0.2f, 0.06f, 0.5f), new Vector3(0.2f, 0.12f, 0.28f), bodyMaterial);
            BodyPart(PrimitiveType.Cube, "PawBack" + suffix, body.transform, new Vector3(side * 0.22f, 0.06f, -0.5f), new Vector3(0.22f, 0.12f, 0.28f), bodyMaterial);
        }

        BodyPart(PrimitiveType.Cube, "Tail", body.transform, new Vector3(0f, 0.88f, -0.84f), new Vector3(0.15f, 0.15f, 0.5f), bodyMaterial, Quaternion.Euler(-26f, 0f, 0f));
        BodyPart(PrimitiveType.Cube, "DetailTailTip", body.transform, new Vector3(0f, 1.04f, -1.1f), new Vector3(0.14f, 0.14f, 0.22f), iceMaterial);
        return body;
    }
    ...
    /// </summary>
    static GameObject BuildWraithBody(Transform parent, Material bodyMaterial, Material eyeMaterial, Material clawMaterial, Material iceMaterial)
    {
        var body = new GameObject("WraithBody");
        body.transform.SetParent(parent, false);

        BodyPart(PrimitiveType.Sphere, "Skirt", body.transform, new Vector3(0f, 0.45f, 0f), new Vector3(1.15f, 0.7f, 1.05f), bodyMaterial);
        BodyPart(PrimitiveType.Sphere, "Robe", body.transform, new Vector3(0f, 1.05f, 0f), new Vector3(0.9f, 1.25f, 0.85f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "DetailChain", body.transform, new Vector3(0f, 0.9f, 0f), new Vector3(0.98f, 0.13f, 0.92f), iceMaterial);
        BodyPart(PrimitiveType.Cube, "Mantle", body.transform, new Vector3(0f, 1.42f, -0.04f), new Vector3(1.05f, 0.3f, 0.9f), bodyMaterial);
        BodyPart(PrimitiveType.Sphere, "Hood", body.transform, new Vector3(0f, 1.78f, 0.02f), new Vector3(0.74f, 0.78f, 0.74f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Brim", body.transform, new Vector3(0f, 1.6f, 0.12f), new Vector3(0.68f, 0.12f, 0.68f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "EyeHollow", body.transform, new Vector3(0f, 1.72f, 0.32f), new Vector3(0.44f, 0.3f, 0.14f), eyeMaterial);

        // A crown of ice around the back of the hood.
        for (int i = 0; i < 5; i++)
        {
            float angle = (i * 40f + 110f) * Mathf.Deg2Rad;
            BodyPart(PrimitiveType.Cube, $"DetailCrown{i}", body.transform,
                new Vector3(Mathf.Cos(angle) * 0.36f, 2.08f, Mathf.Sin(angle) * 0.34f - 0.05f),
                new Vector3(0.12f, 0.42f - Mathf.Abs(i - 2) * 0.06f, 0.12f), iceMaterial, Quaternion.Euler(-Mathf.Sin(angle) * 24f, 45f, Mathf.Cos(angle) * 24f));
        }

        foreach (float side in new[] { -1f, 1f })
        {
            string suffix = side < 0f ? "Left" : "Right";
            BodyPart(PrimitiveType.Sphere, "EyeGlow" + suffix, body.transform, new Vector3(side * 0.13f, 1.74f, 0.4f), new Vector3(0.12f, 0.12f, 0.07f), iceMaterial);

            BodyPart(PrimitiveType.Cube, "Sleeve" + suffix, body.transform, new Vector3(side * 0.54f, 1f, 0.1f), new Vector3(0.26f, 1f, 0.28f), bodyMaterial, Quaternion.Euler(0f, 0f, side * 13f));
        }
        ...
        for (int i = 0; i < 7; i++)
        {
            ...
            BodyPart(PrimitiveType.Cube, $"Rag{i}", body.transform,
                new Vector3(Mathf.Cos(angle) * 0.46f, 0.18f - (i % 2) * 0.08f, Mathf.Sin(angle) * 0.43f),
                new Vector3(0.2f, 0.44f + (i % 2) * 0.18f, 0.2f), bodyMaterial, Quaternion.Euler((i % 2) * 8f, i * 24f, (i % 3 - 1) * 9f));
        }
    ...
    /// </summary>
    static GameObject BuildGolemBody(Transform parent, Material bodyMaterial, Material eyeMaterial, Material clawMaterial, Material iceMaterial)
    {
        var body = new GameObject("GolemBody");
        body.transform.SetParent(parent, false);

        BodyPart(PrimitiveType.Cube, "Hips", body.transform, new Vector3(0f, 0.68f, 0f), new Vector3(0.95f, 0.55f, 0.72f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Torso", body.transform, new Vector3(0f, 1.35f, 0f), new Vector3(1.25f, 1.1f, 0.88f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Shoulders", body.transform, new Vector3(0f, 1.88f, 0f), new Vector3(1.75f, 0.42f, 0.95f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Head", body.transform, new Vector3(0f, 2.15f, 0.06f), new Vector3(0.5f, 0.46f, 0.5f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Brow", body.transform, new Vector3(0f, 2.3f, 0.16f), new Vector3(0.56f, 0.14f, 0.34f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "DetailSeam", body.transform, new Vector3(0f, 1.35f, 0.45f), new Vector3(0.14f, 0.72f, 0.06f), iceMaterial);
        BodyPart(PrimitiveType.Cube, "DetailSeamCross", body.transform, new Vector3(0f, 1.55f, 0.45f), new Vector3(0.5f, 0.1f, 0.06f), iceMaterial);
        BodyPart(PrimitiveType.Cube, "DetailCrack", body.transform, new Vector3(0f, 0.7f, 0.38f), new Vector3(0.34f, 0.08f, 0.06f), clawMaterial);

        // The crystal on its back, one tall shard flanked by two short ones.
        for (int i = 0; i < 3; i++)
        {
            BodyPart(PrimitiveType.Cube, $"DetailCrystal{i}", body.transform,
                new Vector3((i - 1) * 0.32f, 2.0f - Mathf.Abs(i - 1) * 0.24f, -0.52f),
                new Vector3(0.24f, 0.72f - Mathf.Abs(i - 1) * 0.22f, 0.24f), iceMaterial, Quaternion.Euler(22f, 45f, (i - 1) * 16f));
        }

        foreach (float side in new[] { -1f, 1f })
        {
            string suffix = side < 0f ? "Left" : "Right";
            BodyPart(PrimitiveType.Sphere, "Eye" + suffix, body.transform, new Vector3(side * 0.14f, 2.15f, 0.28f), new Vector3(0.12f, 0.12f, 0.06f), iceMaterial);

            BodyPart(PrimitiveType.Cube, "Arm" + suffix, body.transform, new Vector3(side * 0.92f, 1.12f, 0f), new Vector3(0.4f, 1.5f, 0.42f), bodyMaterial);
            BodyPart(PrimitiveType.Cube, "Leg" + suffix, body.transform, new Vector3(side * 0.32f, 0.32f, 0f), new Vector3(0.42f, 0.64f, 0.46f), bodyMaterial);
            BodyPart(PrimitiveType.Cube, "Foot" + suffix, body.transform, new Vector3(side * 0.32f, 0.06f, 0.08f), new Vector3(0.48f, 0.14f, 0.6f), bodyMaterial);

            BodyPart(PrimitiveType.Cube, "ShoulderShard" + suffix, body.transform, new Vector3(side * 0.72f, 2.12f, 0f), new Vector3(0.32f, 0.36f, 0.32f), bodyMaterial, Quaternion.Euler(0f, 45f, side * 18f));
            BodyPart(PrimitiveType.Cube, "DetailShoulderSpike" + suffix, body.transform, new Vector3(side * 0.8f, 2.42f, 0f), new Vector3(0.16f, 0.44f, 0.16f), iceMaterial, Quaternion.Euler(0f, 45f, side * 26f));
        }
    ...
    /// </summary>
    static GameObject BuildToadBody(Transform parent, Material bodyMaterial, Material eyeMaterial, Material emberMaterial)
    {
        var body = new GameObject("ToadBody");
        body.transform.SetParent(parent, false);

        BodyPart(PrimitiveType.Sphere, "Belly", body.transform, new Vector3(0f, 0.42f, 0f), new Vector3(1.05f, 0.66f, 1.05f), bodyMaterial);
        BodyPart(PrimitiveType.Sphere, "Back", body.transform, new Vector3(0f, 0.6f, -0.1f), new Vector3(0.9f, 0.52f, 0.92f), bodyMaterial);
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            BodyPart(PrimitiveType.Sphere, $"DetailWart{i}", body.transform,
                new Vector3(Mathf.Cos(angle) * 0.32f, 0.8f - (i % 2) * 0.08f, Mathf.Sin(angle) * 0.34f - 0.08f),
                new Vector3(0.18f, 0.14f, 0.18f), emberMaterial);
        }
        ...
        foreach (float side in new[] { -1f, 1f })
        {
            ...
            BodyPart(PrimitiveType.Sphere, "Brow" + suffix, body.transform, new Vector3(side * 0.25f, 0.76f, 0.32f), new Vector3(0.3f, 0.3f, 0.3f), bodyMaterial);
            BodyPart(PrimitiveType.Sphere, "Eye" + suffix, body.transform, new Vector3(side * 0.27f, 0.8f, 0.44f), new Vector3(0.16f, 0.16f, 0.1f), eyeMaterial);

            // Hind leg folded up against the body, front foot planted underneath.
            BodyPart(PrimitiveType.Sphere, "Haunch" + suffix, body.transform, new Vector3(side * 0.46f, 0.38f, -0.22f), new Vector3(0.44f, 0.52f, 0.66f), bodyMaterial, Quaternion.Euler(0f, side * 14f, 0f));
            BodyPart(PrimitiveType.Cube, "PawBack" + suffix, body.transform, new Vector3(side * 0.52f, 0.09f, 0.02f), new Vector3(0.3f, 0.18f, 0.56f), bodyMaterial);
            BodyPart(PrimitiveType.Cube, "PawFront" + suffix, body.transform, new Vector3(side * 0.3f, 0.09f, 0.46f), new Vector3(0.28f, 0.18f, 0.38f), bodyMaterial);
        }
    ...
    /// </summary>
    static GameObject BuildScorpionBody(Transform parent, Material bodyMaterial, Material eyeMaterial, Material clawMaterial, Material emberMaterial)
    {
        var body = new GameObject("ScorpionBody");
        body.transform.SetParent(parent, false);

        BodyPart(PrimitiveType.Cube, "Torso", body.transform, new Vector3(0f, 0.5f, -0.15f), new Vector3(0.85f, 0.4f, 1f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Head", body.transform, new Vector3(0f, 0.52f, 0.5f), new Vector3(0.62f, 0.34f, 0.45f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "MandibleLeft", body.transform, new Vector3(-0.14f, 0.44f, 0.76f), new Vector3(0.1f, 0.12f, 0.22f), clawMaterial);
        BodyPart(PrimitiveType.Cube, "MandibleRight", body.transform, new Vector3(0.14f, 0.44f, 0.76f), new Vector3(0.1f, 0.12f, 0.22f), clawMaterial);

        // Shell plates down the back, with the heat showing in the gaps.
        for (int i = 0; i < 3; i++)
        {
            BodyPart(PrimitiveType.Cube, $"Plate{i}", body.transform,
                new Vector3(0f, 0.72f, 0.2f - i * 0.36f), new Vector3(0.78f - i * 0.06f, 0.2f, 0.3f), bodyMaterial);
            BodyPart(PrimitiveType.Cube, $"DetailGlow{i}", body.transform,
                new Vector3(0f, 0.68f, 0.02f - i * 0.36f), new Vector3(0.8f - i * 0.06f, 0.1f, 0.07f), emberMaterial);
        }
        ...
        foreach (float side in new[] { -1f, 1f })
        {
            ...
            BodyPart(PrimitiveType.Sphere, "Eye" + suffix, body.transform, new Vector3(side * 0.15f, 0.63f, 0.7f), new Vector3(0.1f, 0.1f, 0.06f), emberMaterial);
            BodyPart(PrimitiveType.Sphere, "EyeSmall" + suffix, body.transform, new Vector3(side * 0.26f, 0.6f, 0.62f), new Vector3(0.07f, 0.07f, 0.05f), eyeMaterial);

            // Pincer: an arm reaching forward, a claw split in two, and a spur on the elbow.
            BodyPart(PrimitiveType.Cube, "Arm" + suffix, body.transform, new Vector3(side * 0.52f, 0.46f, 0.62f), new Vector3(0.2f, 0.2f, 0.62f), bodyMaterial, Quaternion.Euler(0f, side * 18f, 0f));
            BodyPart(PrimitiveType.Cube, "ClawUpper" + suffix, body.transform, new Vector3(side * 0.68f, 0.57f, 1.02f), new Vector3(0.17f, 0.15f, 0.44f), clawMaterial);
            BodyPart(PrimitiveType.Cube, "ClawLower" + suffix, body.transform, new Vector3(side * 0.68f, 0.38f, 0.98f), new Vector3(0.17f, 0.15f, 0.38f), clawMaterial);
            BodyPart(PrimitiveType.Cube, "ClawSpur" + suffix, body.transform, new Vector3(side * 0.5f, 0.6f, 0.4f), new Vector3(0.1f, 0.24f, 0.1f), clawMaterial);

            for (int leg = 0; leg < 3; leg++)
            {
                BodyPart(PrimitiveType.Cube, $"Leg{suffix}{leg}", body.transform,
        ...
        {
            float t = i / 4f;
            BodyPart(PrimitiveType.Cube, $"Tail{i}", body.transform,
                new Vector3(0f, 0.55f + t * 0.95f, -0.75f - t * 0.3f + t * t * 0.55f),
                new Vector3(0.3f - t * 0.09f, 0.28f - t * 0.07f, 0.32f), bodyMaterial, Quaternion.Euler(-28f - i * 16f, 0f, 0f));
            BodyPart(PrimitiveType.Cube, $"DetailTailGlow{i}", body.transform,
                new Vector3(0f, 0.55f + t * 0.95f, -0.62f - t * 0.3f + t * t * 0.55f),
                new Vector3(0.14f - t * 0.03f, 0.08f, 0.06f), emberMaterial);
        }

        BodyPart(PrimitiveType.Cube, "ClawStinger", body.transform, new Vector3(0f, 1.6f, -0.42f), new Vector3(0.2f, 0.4f, 0.2f), clawMaterial, Quaternion.Euler(35f, 45f, 0f));
        BodyPart(PrimitiveType.Cube, "DetailVenom", body.transform, new Vector3(0f, 1.4f, -0.3f), new Vector3(0.12f, 0.16f, 0.12f), emberMaterial);
        return body;
    }
    ...
    /// </summary>
    static GameObject BuildWispBody(Transform parent, Material bodyMaterial, Material eyeMaterial, Material emberMaterial, Material clawMaterial)
    {
        var body = new GameObject("WispBody");
        body.transform.SetParent(parent, false);

        BodyPart(PrimitiveType.Sphere, "Core", body.transform, new Vector3(0f, 1.15f, 0f), new Vector3(0.8f, 0.9f, 0.8f), emberMaterial);
        ...
        {
            (new Vector3(0f, 1.5f, -0.06f), new Vector3(0.72f, 0.4f, 0.72f), new Vector3(0f, 20f, 0f)),
            (new Vector3(-0.3f, 1.05f, -0.24f), new Vector3(0.5f, 0.7f, 0.5f), new Vector3(0f, 35f, 14f)),
            (new Vector3(0.32f, 1.0f, -0.2f), new Vector3(0.48f, 0.66f, 0.48f), new Vector3(0f, -30f, -12f)),
            (new Vector3(0f, 0.78f, 0.06f), new Vector3(0.66f, 0.34f, 0.62f), new Vector3(8f, 15f, 0f)),
        };
        for (int i = 0; i < plates.Length; i++)
        {
            BodyPart(PrimitiveType.Cube, $"DetailCrust{i}", body.transform, plates[i].position, plates[i].scale, clawMaterial, Quaternion.Euler(plates[i].rotation));
        }

        BodyPart(PrimitiveType.Sphere, "EyeLeft", body.transform, new Vector3(-0.17f, 1.34f, 0.33f), new Vector3(0.13f, 0.13f, 0.08f), eyeMaterial);
        BodyPart(PrimitiveType.Sphere, "EyeRight", body.transform, new Vector3(0.17f, 1.34f, 0.33f), new Vector3(0.13f, 0.13f, 0.08f), eyeMaterial);
        BodyPart(PrimitiveType.Cube, "DetailMouth", body.transform, new Vector3(0f, 1.12f, 0.36f), new Vector3(0.3f, 0.09f, 0.06f), emberMaterial);

        // Shards held in orbit, and cinders drifting between them.
        for (int i = 0; i < 6; i++)
        {
            float angle = i * 60f * Mathf.Deg2Rad;
            BodyPart(PrimitiveType.Cube, $"Shard{i}", body.transform,
                new Vector3(Mathf.Cos(angle) * 0.72f, 0.95f + (i % 3) * 0.3f, Mathf.Sin(angle) * 0.72f),
                new Vector3(0.22f, 0.36f - (i % 3) * 0.06f, 0.22f), bodyMaterial, Quaternion.Euler(i * 19f, 45f, 22f + i * 7f));

            BodyPart(PrimitiveType.Cube, $"DetailCinder{i}", body.transform,
                new Vector3(Mathf.Cos(angle + 0.5f) * 0.5f, 0.7f + (i % 2) * 0.9f, Mathf.Sin(angle + 0.5f) * 0.5f),
                new Vector3(0.1f, 0.1f, 0.1f), emberMaterial);
        }
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            BodyPart(PrimitiveType.Cube, $"DetailFlame{i}", body.transform,
                new Vector3(offset, 1.78f + (2 - Mathf.Abs(i - 2)) * 0.14f, -Mathf.Abs(offset) * 0.4f),
                new Vector3(0.17f, 0.44f + (2 - Mathf.Abs(i - 2)) * 0.22f, 0.17f), emberMaterial, Quaternion.Euler(0f, 45f, offset * 40f));
        }

        BodyPart(PrimitiveType.Cube, "DetailAnchor", body.transform, new Vector3(0f, 0.36f, 0f), new Vector3(0.34f, 0.3f, 0.34f), clawMaterial);
        return body;
    }
    ...
    /// </summary>
    static GameObject BuildOgreBody(Transform parent, Material bodyMaterial, Material eyeMaterial, Material boneMaterial, Material clawMaterial)
    {
        ...
        BodyPart(PrimitiveType.Cube, "Brow", body.transform, new Vector3(0f, 2.66f, 0.3f), new Vector3(0.66f, 0.16f, 0.28f), bodyMaterial);
        BodyPart(PrimitiveType.Cube, "Jaw", body.transform, new Vector3(0f, 2.24f, 0.28f), new Vector3(0.58f, 0.24f, 0.5f), bodyMaterial);
        ...
        foreach (float side in new[] { -1f, 1f })
        {
            ...
            BodyPart(PrimitiveType.Cube, "Horn" + suffix, body.transform, new Vector3(side * 0.34f, 2.85f, 0.05f), new Vector3(0.16f, 0.46f, 0.16f), boneMaterial);
            BodyPart(PrimitiveType.Cube, "HornTip" + suffix, body.transform, new Vector3(side * 0.44f, 3.12f, -0.1f), new Vector3(0.12f, 0.34f, 0.12f), boneMaterial, Quaternion.Euler(-28f, 0f, side * 32f));
    ...
    static GameObject BuildFrostGiantBody(Transform parent, Material bodyMaterial, Material eyeMaterial, Material iceMaterial)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            BodyPart(PrimitiveType.Cube, $"DetailCrown{i}", body.transform,
                ...
                new Vector3(0.16f, 0.55f, 0.16f), iceMaterial, Quaternion.Euler(Mathf.Sin(angle) * 22f, 45f, -Mathf.Cos(angle) * 22f));
        }
        ...
        for (int i = 0; i < 4; i++)
        {
            BodyPart(PrimitiveType.Cube, $"DetailSpine{i}", body.transform,
                ...
                new Vector3(0.2f, 0.7f - i * 0.1f, 0.2f), iceMaterial, Quaternion.Euler(-30f - i * 6f, 45f, 0f));
        }
    ...
    static GameObject BuildLavaBruteBody(Transform parent, Material bodyMaterial, Material eyeMaterial, Material emberMaterial, Material clawMaterial)
    {
        ...
        {
            string suffix = side < 0f ? "Left" : "Right";
            BodyPart(PrimitiveType.Sphere, "EyeGlow" + suffix, body.transform, new Vector3(side * 0.26f, 2.16f, 0.94f), new Vector3(0.17f, 0.17f, 0.1f), emberMaterial);
    ...
    static GameObject BuildSword(Transform armPivot, string name, Color blade, Color trim)
    {
        ...
        foreach (float side in new[] { -1f, 1f })
        {
            BodyPart(PrimitiveType.Cube, side < 0f ? "GuardTipLeft" : "GuardTipRight", sword.transform,
                new Vector3(side * 0.17f, -0.145f, 0f), new Vector3(0.07f, 0.09f, 0.09f), trimMaterial, Quaternion.Euler(0f, 0f, side * 28f));
        }
    ...
    static GameObject BodyPart(PrimitiveType type, string name, Transform parent, Vector3 localPosition, Vector3 localScale, Material material, Quaternion rotation)
    {
    ...
    static GameObject CreatePrimitive(PrimitiveType type, string name, Transform parent, Vector3 localPosition, Vector3 localScale, Material material, Quaternion rotation)
    {
        var part = CreatePrimitive(type, name, parent, localPosition, localScale, material);
        part.transform.localRotation = rotation;
        return part;
